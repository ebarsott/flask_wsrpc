...
class JSONRPC(object):
    ...
    def _receive(self):
        ...
        try:
            ...
            logging.debug("decoded {}".format(dm))
        except errors.RPCError as e:
            logging.error("decode error {}".format(e), exc_info=e)
            self._send(e)
            return None
    ...
    def _process(self, m):
        try:
                ...
                def cb(*args, **kwargs):
                    ...
                    try:
                        ...
                        try:
                            ...
                            pass
                    logging.debug('signal callback done')
                logging.debug('connecting to signal {} with id {}'.format(
                    m['method'], m['id']))
                obj.connect(cb)
                ...
                if len(m['params']) != 1:  # TODO instead disconnect all?
                    return self._send(errors.ServerError(
                        'Invalid params {} expected 1 length array'.format(
                            m['params']), m['id']))
                cbid = m['params'][0]
                logging.debug('disconnecting from signal {} with id {}'.format(
                    m['method'], cbid))
            ...
            logging.error("call error {}".format(e), exc_info=e)
            if m['id'] is None:
        ...
        if isinstance(res, concurrent.futures.Future):
            ...
            logging.debug("returning future {}".format(future))
            return dict(jsonrpc='2.0', result=future, id=m['id'])
        else:
            logging.debug("returning {}".format(res))
    ...
    def update(self):
        r = self._receive()
        logging.debug("received r: {}".format(r))
